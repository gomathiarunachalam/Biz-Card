...
with tab2:
    ...
    with col1:
        ...
        try:
            ...
            cardholder_name = st.selectbox("**Select a Cardholder name to Edit the details**", names,
                                           key='cardholder_name')

            # Collect all data depending on the cardholder's name
            cursor.execute(
                "SELECT Company_name, Card_holder, Designation, Mobile_number, Email, Website, Area, City, State, Pin_code FROM bizcardx_data WHERE card_holder=%s",
                (cardholder_name,))
    ...
    with col2:
        ...
        try:
            ...
            # Create a selection box to select cardholder name
            delete_name = st.selectbox("**Select a Cardholder name to Delete the details**", del_names,
                                       key='delete_name')
